...
def load_model_from_sftp(username, max_retries=3, timeout=30):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            logging.info(f"🔄 Connecting to SFTP: {SFTP_HOST} as {SFTP_USERNAME} (Attempt {attempt}/{max_retries})...")
            transport = paramiko.Transport((SFTP_HOST, 22))
            transport.connect(username=SFTP_USERNAME, password=SFTP_PASSWORD)
            sftp = paramiko.SFTPClient.from_transport(transport)
            sftp.get_channel().settimeout(timeout)  # ✅ Set timeout
            ...
            logging.error(f"⚠️ SFTP Error: {str(e)} (Retry {attempt}/{max_retries})")
        except Exception as e:
            logging.error(f"⚠️ Unknown Error: {str(e)} (Retry {attempt}/{max_retries})")
    ...
    logging.error("❌ SFTP connection failed after multiple retries")
